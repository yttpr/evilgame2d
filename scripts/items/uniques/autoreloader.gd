...
func _process(delta: float) -> void:
	...
	for i in amt:
		if i == Manager.current_gun_index:
			continue
		if Manager.Player.weapon_handler.remaining_clips[i] < ceili(Manager.Player.items._check_items("MaxClip", Manager.Player.weapon_handler.weapons[i].clip_size, Manager.Player.weapon_handler.weapons[i], self)):
			if Manager.Player.weapon_handler.reload_lefts[i] <= 0.0:
				Manager.Player.weapon_handler.reload_lefts[i] = Manager.Player.weapon_handler.weapons[i].reload_time
